...
namespace xcas {
  ...
  Graph3d::Graph3d(int x,int y,int width, int height,const char* title,History_Pack * hp_): 
    ...
    legende_size=max(min(legende_size,width/4),width/5);
...
{63552,47168,30752,14336},
  ...
  void Graph3d::update_rotation(){
    ...
    for (int i=0;i<v.size();++i){
      int u=default_upcolor,d=default_downcolor,du=default_downupcolor,dd=default_downdowncolor; pal=-1;
      const char * ptr=0;
	...
	if (attrv.size()>1){
	  gen attr=attrv[1];
          if (attr.type==_INT_ && attr.val!=default_color(contextptr) && is_colormap_index(attr.val & 0xffff)){
            ...
            for (int i=0;i<N;++i){
              int4 I; int c,r,g,b;
              if (colormap_color_rgb(pal,i/(N-1.0),c,r,g,b,contextptr)){
	...
	if (h.type==_VECT && h.subtype==_POLYEDRE__VECT)
	  ...
	  int4 I41={0,0,0,pal},I42={u,d,du,dd};
  ...
  bool Graph3d::indraw3d(int w,int h,int lcdz,GIAC_CONTEXT,
	...
	polyedreymax.push_back(ymax);
      ...
      for (int k=0;k<int(hypv.size());k+=2){
        bool cplx=hyp_color[k].u==0 && hyp_color[k].d==0 && hyp_color[k].du==0 ;
        int pal=-1;
        if (cplx)
          pal=hyp_color[k].dd;
	  ...
	  for (itprec+=3,itcur+=3;itprec<itprecend;itprec+=3,itcur+=3){
	    ...
	    if ( Z>=window_zmin && Z<=window_zmax &&
	      ...
	      if (cplx){
                ...
                if (pal>=0) 
                  res.colorptr=&tabcolormap[idx];
                else
                  res.colorptr=&tabcolorcplx[idx];
                //CERR << a1 << " " << idx << " " << res.colorptr->u << "," << res.colorptr->d << "; ";
	      }
	      else
