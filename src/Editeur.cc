...
//
  ...
  void send_numworks(const string & name_,const string & res){
    ...
    int nwstoresize=dfu_get_scriptstore(backup);
    if (!nwstoresize){
    ...
    m[name]=r;
    if (!map2scriptstore(m,backup,nwstoresize)){
  ...
  void update_nws_flash(const char * buf,Fl_Hold_Browser * br){
    ...
    if (m.empty()){
      ...
      if (it->filename=="khi110ab" || it->filename=="khi120ab"){
        ++it;
        if (it->filename=="khicas.icon")
          ++it;
      }
      for (;it!=itend;++it){
  ...
  void nws_flash(){
	...
	buf=newbuf;
        ...
        if (memcmp(buf+0x100,star,6)){
          if (memcmp(buf+0x200000+0x100,star,6))
            continue; // invalid
        }
	update_nws_flash(buf,br);
	continue;
