...
namespace xcas {
  ...
  int tableur_minsize_for_graph=320;
...
#ifdef EMCC2
  int Flv_Table_Gen::def_rows=20,Flv_Table_Gen::def_cols=5;
#else
  ...
  */
  ...
  void cb_Tableur_Graph2d(Fl_Widget* m , void*) {
    Flv_Table_Gen * tg=find_table_brother(m);
#if 1 // def EMCC2
    Tableur_Group * gr = dynamic_cast<Tableur_Group *>(tg->parent());
    if (gr->disposition&2)
      gr->disposition &= 1;
    else
      gr->disposition |= 2;
  ...
  bool tablefunc_dialog(Fl_Widget * spread_ptr,std::string & arg,bool plot,int type,const string & title){
    if (!Xcas_MainTab || !Xcas_MainTab->visible()){ fl_alert("Please finish current task"); return false; }
#ifdef EMCC2
    Xcas_main_menu_hide();
#endif
#ifdef MODAL_GROUP
    ...
    static Fl_Group * methode=0;
    static Fl_Button * button1 =0,*m1=0,*m2=0,*m3=0,*m4=0,*m5=0;
	...
	dy=spread_ptr?8*spread_ptr->labelsize():400;
#endif
      int lignes=7;
#ifdef MODAL_GROUP
      Fl_Group::current(Xcas_Main_Window);
      w=plot_w=new Fl_Group(0,0,dx,dy);
      modal_w_tab.push_back(plot_w);
#else
      Fl_Group::current(0);
      w=new Fl_Window(dx,dy);
#endif      
      ...
      button0 = new Fl_Return_Button(2,2+6*dy/lignes,dx/2-4,dy/lignes-4);
      button0->shortcut(0xff0d);
      button0->label(gettext("OK"));
      button1 = new Fl_Button(dx/2+2,2+6*dy/lignes,dx/2-4,dy/lignes-4);
      button1->shortcut(0xff1b);
      button1->label(gettext("Cancel"));
      Xcas_Methodes_Output = new Fl_Output(4*dx/6,2+3*dy/lignes,dx/3-2,dy/lignes-4,gettext("int. method"));
      Xcas_Methodes_Output->value(gettext("builtin"));
      // methode = new Fl_Menu_Button(0,2+3*dy/lignes,dx/3-2,dy/lignes-4,gettext("Choose")); methode->menu(Xcas_Menu_Methodes);
      methode = new Fl_Group(0,2+3*dy/lignes,dx-2,3*dy/lignes-4);
      m1 = new Fl_Button(0,2+3*dy/lignes,dx/2-4,dy/lignes-4);
      m1->label(gettext("builtin"));
      m1->callback( (Fl_Callback*)cb_Xcas_Methodes_builtin);
      m2 = new Fl_Button(2,2+4*dy/lignes,dx/2-4,dy/lignes-4);
      m2->label(gettext("right_rectangle"));
      m2->callback( (Fl_Callback*)cb_Xcas_Methodes_right_rectangle);
      m3 = new Fl_Button(dx/2+2,2+4*dy/lignes,dx/2-4,dy/lignes-4);
      m3->label(gettext("left_rectangle"));
      m3->callback( (Fl_Callback*)cb_Xcas_Methodes_left_rectangle);
      m4 = new Fl_Button(2,2+5*dy/lignes,dx/2-4,dy/lignes-4);
      m4->label(gettext("trapezoid"));
      m4->callback( (Fl_Callback*)cb_Xcas_Methodes_trapezoid);
      m5 = new Fl_Button(dx/2+2,2+5*dy/lignes,dx/2-4,dy/lignes-4);
      m5->label(gettext("middle_point"));
      m5->callback( (Fl_Callback*)cb_Xcas_Methodes_middle_point);
      methode->end();
      w->end();
    ...
    //w->label(title.c_str()); w->labeltype(FL_ALIGN_BOTTOM);
   ...
   void cb_Tableur_Tablefunc(Fl_Menu_* m , void*) {
    ...
    if (spread_ptr && tablefunc_dialog(spread_ptr,arg,false,0,gettext("Table of value of a function"))){
      ...
      if (gr && gr->h()>=tableur_minsize_for_graph && !(gr->disposition & 2) && !spread_ptr->graph2d->window()->visible() ){
	gr->disposition |= 2;
	gr->resize2();
      }
  ...
  bool tableseq_dialog(Fl_Widget * spread_ptr,std::string & arg,bool plot,const string & title,std::string & u0param){
	...
	fig->geo->orthonormalize();
      ...
      if (plot)
	arg += "="+u0s+","+print_DOUBLE_(nterms->value());// string("=[")+u0s+string(",")+print_DOUBLE_(xmin->value())+string(",")+print_DOUBLE_(xmax->value())+string("],")+print_DOUBLE_(nterms->value());
      else
  ...
  int Flv_Table_Gen::set_graphic_dialog(const string & title,const string & aide,gen & in, gen & out,bool show_class,bool & absolu,bool & transpose,double & degree){
    ...
    */
	...
	Tableur_Group * gr = dynamic_cast<Tableur_Group *>(parent());
	if (!is_undef(out) && gr && gr->h()>=tableur_minsize_for_graph && !(gr->disposition & 2) ){
	  gr->disposition |= 2;
	  gr->resize2();
	}
      }
...
#ifdef EMCC2
    global_style.width(90);
#else // IPAQ
    global_style.width(10+7*labelsize());
#endif // IPAQ
  ...
  */
  ...
  Tableur_Group::Tableur_Group(int X,int Y,int W,int H,int L,int disp):Fl_Tile(X,Y,W,H),disposition(disp),dtable(0),dgraph(0),dparam(0){
    labelsize(L);
    int l=L+(H<400?4:6),inputh=L+(H<400?12:18);
    ...
    menubar = new Fl_Menu_Bar(X,Y,5*l,l);
  ...
  void Tableur_Group::resize2(double dt,double dg,double dp){
    ...
    int L=labelsize();
    int l=L+(H<400?4:6),inputh=L+(H<400?12:18);
    menubar->resize(X,Y,0.42*W,l);
    reevalsave->resize(X+menubar->w(),Y,W-menubar->w(),l);
    if (disposition/2){
