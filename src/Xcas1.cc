...
namespace xcas {
  ...
  void Xcas_DispG_Window_show(){
    if (Xcas_DispG_Window){
      Xcas_DispG_Window->show();
      int n=Xcas_DispG_Window->children();
      if (Xcas_DispG && n>3){
        Fl_Widget * wid=Xcas_DispG_Window->child(3);
        if (dynamic_cast<Fl_Button *>(wid)){
          static string clears;
          clears=gettext("Clear")+(" "+giac::print_INT_(Xcas_DispG->plot_instructions.size())+"/"+giac::print_INT_(Xcas_DispG->max_displayed));
          wid->label(clears.c_str());
        }
      }
    }
  }
  ...
  void xcas_gprintf(unsigned special,const std::string & format,const vecteur & v,GIAC_CONTEXT){
    if ( ((int)special) >= 0){
      ...
      for (int i=0;i<v.size();++i){
        ...
        as += v[i].print(contextptr);
      }
      *logptr(contextptr) << as << "\n";
    }
    if (Xcas_PrintG){
      Fl::lock();
  ...
  void Xcas_debugguer(int status,giac::context * contextptr){
    ...
    if (status==3){
      ...
      if (v.empty()){
	if (Xcas_DispG_Window) Xcas_DispG_Window_show();
  ...
  std::string widget_html5(const Fl_Widget * o,int & pos){
    ...
    if (const Editeur * ed=dynamic_cast<const Editeur *>(o)){
      return widget_html5(ed->editor,pos);
      // old code
  ...
  bool fltk_view(const giac::gen & g,giac::gen & ge,const std::string & filename,std::string & figure_filename,int file_type,const giac::context *contextptr){
    ...
    if (xcas::Graph2d3d * this_graph = dynamic_cast<xcas::Graph2d3d *>(wid)){
      ...
      if (Graph2d * gr=dynamic_cast<Graph2d *>(this_graph)){
        gr->init_tracemode(); gr->tracemode_set();
      }
    }
