...
#ifdef EMCC2
#include "hist.h"
#define kbd_drag 1 // this will work only if evaluation is not in a separate thread
#else
#define kbd_drag 0
#endif
...
namespace xcas {
  ...
  Fl_Menu_Item Graph2d3d_menu[] = {
    ...
    {gettext("Clear info"), 0,  (Fl_Callback *) cb_Graph_clear_infos, 0, 0, 0, 0, 14, 56},
    ...
    {gettext("x'=0, vert. tangent"), 0,  (Fl_Callback *) cb_Graph_vertical, 0, 0, 0, 0, 14, 56},
  ...
  void Graph2d3d::add_mouse_param_group(int x,int y,int w,int h){
    ...
    int bx=x+w-legende_size,by=y+mouse_position->h(),bw=legende_size/3,bh=min(group->labelsize()+10,h/6);
  ...
  Graph2d3d::Graph2d3d(int x,int y,int w,int h,const char * l,double xmin,double xmax,double ymin,double ymax,double zmin,double zmax,double ortho,History_Pack * hp_):
    ...
    legende_size=max(min(legende_size,w/4),w/5);
    x_axis_color=FL_RED;
  ...
  // round window_xmin/xmax to wxmin/max
  void set_pminmax(double window_xmin,double window_xmax,double & wxmin,double & wxmax){
    double x_tick=find_tick(window_xmax-window_xmin);
    wxmin=int( window_xmin/x_tick -1)*x_tick;
    wxmax=int( window_xmax/x_tick +1)*x_tick;
  }    
  ...
  void Graph2d3d::add(const gen & e){ 
    ...
    if ( (dynamic_cast<Geo2d *>(this) || dynamic_cast<Geo3d *>(this)) && param_group && (gvs=parameter2slider(e,contextptr)) ){
      ...
      int x=param_group->x(), y=param_group->y(),h=param_group->h(),w=0.73*param_group->w();
      int nmax=int(0.5+param_group->h()/(1.75*labelsize()));
  ...
  void Graph2d3d::resize_mouse_param_group(int W){
    if (mouse_param_group){
      ...
      y += 4.35*bh;
      {
        ...
        int x=param_group->x(), y=param_group->y(),h=param_group->h(),w=0.73*param_group->w();
        for (int i=0;i<param_group->children();++i){
	  param_group->child(i)->resize(x,param_group->child(i)->y(),w,param_group->child(i)->h());          
        }
      }
  ...
  void Graph2d3d::adjust_cursor_point_type(){
    context * contextptr=hp?hp->contextptr:get_context(this);
    if (kbd_drag || abs_calc_mode(contextptr)==38){
  ...
  int Graph2d::common_in_handle(int event){
    ...
    if (event==FL_MOVE || event==FL_DRAG || event==FL_PUSH || event==FL_RELEASE){
      if (event!=FL_MOVE || !kbd_drag || !moving){
    ...
    if (mode!=255 && (kbd_drag || abs_calc_mode(contextptr)==38) && event==FL_KEYBOARD){
      switch (Fl::event_key()){
      case FL_Right:
	...
	if (ch>='A' && ch<='Z'){
          //cout << "char " << ch << "\n";
	  gen tmp=gen(string(abs_calc_mode(contextptr)==38?"G":"")+ch,contextptr);
          //cout << "id " << tmp << "\n";
	  if (tmp.type==_IDNT){
	...
	if (s){
	  ...
	  if (r!=1){
		...
		if (formel){
		  ...
		  if (v3s>0){
                    double p,P; set_pminmax(window_xmin,window_xmax,p,P);
		    savev.push_back(makevecteur(v3[0],p,P));
		    v3[0]=gen(pntname.print(contextptr)+"x",hp->contextptr);
		  }
		  if (v3s>1){
                    double p,P; set_pminmax(window_ymin,window_ymax,p,P);
		    savev.push_back(makevecteur(v3[1],p,P));
		    v3[1]=gen(pntname.print(contextptr)+"y",hp->contextptr);
		  }
		  if (v3s>2){
                    double p,P; set_pminmax(window_zmin,window_zmax,p,P);
		    savev.push_back(makevecteur(v3[2],p,P));
...
#if 1
    Fl_Group * gr=parent();
    Figure * fig=dynamic_cast<Figure*>(gr);
    if (gr){
      int dW=0;
      if (hp){
        if (fig){
      ...
      int W2=gr->w()-legende_size-dW,W=w();
      if (W!=W2){
        legende_size=max(min(legende_size,gr->w()/5),gr->w()/6);
        int X=x();
        if (fig && fig->disposition==0){ // resize geometry history pack
          dW=hp->w();
          double maxhpratio=0.38;
          if (dW<fig->w()/4)
            dW=fig->w()/4;
          else if (dW>maxhpratio*fig->w())
            dW=maxhpratio*fig->w();
          fig->s->resize(fig->s->x(),fig->s->y(),dW,fig->s->h());
          hp->Fl_Group::resize(hp->x(),hp->y(),dW-labelsize(),hp->h());
          X=fig->s->x()+dW;
        }
        W2=gr->w()-legende_size-dW;
        resize(X,y(),W2,h());
  ...
  void Geo2d::draw(){
    ...
    int horizontal_pixels=horz_pixels();
    int deltax=x(),deltay=y();
    if (mode<255 && (kbd_drag || abs_calc_mode(contextptr)==38)){
      fl_color(FL_CYAN);
  ...
  void draw_legende(const vecteur & f,int i0,int j0,int labelpos,const Graph2d * iptr,int clip_x,int clip_y,int clip_w,int clip_h,int deltax,int deltay){
    ...
    if (legendes.empty())
      return;
    if ((kbd_drag ||abs_calc_mode(contextptr)==38) && legendes.size()>1 && legendes[0]=='G')
  ...
  unsigned char * set_pixel(unsigned char * pixels,int x,int y,int w,int h,int R,int G,int B,double alpha){
    if (x<0 || x>=w || y<0 || y>=h) return 0;
    unsigned char *r=pixels+3*(y*w+x);
    r[0]=alpha*r[0]+R;
    r[1]=alpha*r[1]+G;
    r[2]=alpha*r[2]+B;
    return r+3;
  ...
  unsigned char * set_pixel(unsigned char * r,int R,int G,int B,double alpha){
    r[0]=alpha*r[0]+R;
    r[1]=alpha*r[1]+G;
    r[2]=alpha*r[2]+B;
    return r+3;
  ...
  void color2rgb(int color,double alpha,unsigned char &R,unsigned char & G,unsigned char & B){
    if (color<256){
      Fl::get_color(color, R, G, B);
    }
    else {
      int color888=rgb565to888(color);
      R=(color888 & 0xff000000)>>24;
      G=(color888 & 0xff0000)>>16;
      B=(color888 & 0xff00)>>8;
    }
    R=(1-alpha)*R;
    G=(1-alpha)*G;
  ...
  void alpha_draw_filled_polygon(vector< vector<int> > &L,int xmin,int xmax,int ymin,int ymax,int dy,int color,double alpha,unsigned char * pixels){
    int W=xmax-xmin,H=ymax-ymin;
    unsigned char R,G,B;
    color2rgb(color,alpha,R,G,B);
    int n=L.size();
    ...
    // main loop
    for (;y<ymax;){
      if (y>=ymin+dy){ // draw pixels for this horizontal frame
	...
	for (size_t k=0;k<lxjs;++k){
	  if (impair[k]){
	    ...
	    int x2=k==lxjs-1?xmax:giacmin(xmax,int(lxj[k+1][0]+.5));
            unsigned char * ptr=set_pixel(pixels,x1-xmin,y-ymin+dy,W,H,R,G,B,alpha);
            if (ptr){
              for (++x1;x1<=x2;++x1)
                ptr=set_pixel(ptr,R,G,B,alpha);
            }
	  }
	}
      } // end if y>=ymin
  ...
  void alpha_draw_filled_arc(int x,int y,int rx,int ry,int theta1_deg,int theta2_deg,int xmin,int xmax,int ymin,int ymax,bool segment,int dy,int color,double alpha,unsigned char * pixels){
    ...
    v.back()=v.front();
    alpha_draw_filled_polygon(v,xmin,xmax,ymin,ymax,dy,color,alpha,pixels);
  }    

  // [(X-x)/rx]^2 + [(Y-y)/ry]^2 <= 1, arg(X-x+i*(Y-y) in theta1, theta2
  void alpha_pie(int x,int y,int rx,int ry,int theta1_deg,int theta2_deg,int xmin,int xmax,int ymin, int ymax,int dy,int color,double alpha,unsigned char * pixels){
    // FIXME faster implementation if rx==ry and specific values of theta
    alpha_draw_filled_arc(x,y,rx,ry,theta1_deg,theta2_deg,xmin,xmax,ymin,ymax,false,dy,color,alpha,pixels);
  }
	...
	*/
	if (point._SYMBptr->sommet==at_cercle && !(Mon_image.display_mode & 0xc00)){
	  ...
	  if ( (diam.type==_DOUBLE_) && (a1.type==_DOUBLE_) && (a2.type==_DOUBLE_) ){
	    ...
	    anglef = anglei + diff;
	    if (fill_polygon){
              bool fullcircle = v[1]==0 && v[2]==cst_two_pi;
              int xx=Mon_image.x(),yy=Mon_image.y(),ww=Mon_image.w(),hh=Mon_image.h(),HH=Mon_image.window()->h();
              if (yy+hh>HH)
                hh=HH-yy;
              unsigned char * pixels=0;
              if (epaisseur_point>2){
                if (yy>=0)
                  pixels=fl_read_image(0,xx,yy,ww,hh);
                else {
                  hh += yy;
                  pixels=fl_read_image(0,xx,0,ww,hh);
                }
              }
              if (pixels){
                if (fullcircle)
                  alpha_pie(round(i0),round(j0),round(i1),round(j1),0,360,0,ww,0,hh,yy<0?yy:0,couleur,(epaisseur_point-2)/7.0,pixels);
                else
                  alpha_pie(deltax+round(i0-i1),deltay+round(j0-j1),round(2*i1),round(2*j1),anglei*180/M_PI,anglef*180/M_PI,0,ww,0,hh,yy<0?yy:0,couleur,(epaisseur_point-2)/7.0,pixels);
                if (yy>=0)
                  fl_draw_image(pixels,xx,yy,ww,hh);
                else
                  fl_draw_image(pixels,xx,0,ww,hh);            
                delete [] pixels;
              }
	      else {
                if (fullcircle)
	...
	return;
      ...
      if (jt->type==_VECT)
	return;
      if ( (type_point || (!fill_polygon && epaisseur_point>2)) && type_line==0 && width==0){
	for (;jt!=jtend;++jt){
	...
	return;
      ...
      if (fill_polygon){
        // screen sizes: Fl;;w() and Fl::h()
        // multi-screen Fl::screen_xywh(x, y, w, h, 0);
        // Fl::screen_work_area(x, y, w, h, 0);
        // Fl::screen_count()
        int xx=Mon_image.x(),yy=Mon_image.y(),ww=Mon_image.w(),hh=Mon_image.h(),XX,YY,WW,HH;
        YY=0;
        YY+=yy;
        HH=Mon_image.window()->h(); // Fl::h();// Fl::screen_work_area(XX, YY, WW, HH, 0);
        if (YY+hh>HH) hh=HH-YY;
        unsigned char * pixels=0;
        if (epaisseur_point>2 && jtend-jt>=3 ){
          if (yy>=0)
            pixels=fl_read_image(0,xx,yy,ww,hh);
          else {
            hh += yy;
        ...
        if (pixels){
          // use epaisseur_point in 1..7 for transparency, alpha=epaisseur_pont/7.0
          // composition=alpha*background+(1-alpha)*color
          // get background fl_read_image          
          vector< vector<int> > XY; XY.reserve(jtend-jt+1); vector<int> xy(2);
          xy[0]=i0;xy[1]=j0;
          XY.push_back(xy);
          for (++jt;jt!=jtend;++jt){
          ...
          alpha_draw_filled_polygon(XY,0,ww,0,hh,yy<0?yy:0,couleur,(epaisseur_point-2)/7.0,pixels);          
          // draw composite
          if (yy>=0)
            fl_draw_image(pixels,xx,yy,ww,hh);
          else
  ...
  void Graph2d3d::tracemode_set(int operation){
    ...
    if (G.type==_VECT){
      vecteur & v=*G._VECTptr;
      if (v.size()>16){
        gen x2(eval(*x2ptr(),1,contextptr));
    ...
    if (Gx.type==_DOUBLE_ && Gy.type==_DOUBLE_){
      ...
      if (implicit){
        ...
        gen m(eval(makevecteur(*x1ptr(),*y1ptr()),1,contextptr));
  ...
  void Graph2d::in_draw(int clip_x,int clip_y,int clip_w,int clip_h,int & vertical_pixels){
    ...
    if (display_mode & 1) {
      ...
      if (max_displayed>0 && atend-at>max_displayed)
        at=atend-max_displayed;
  ...
  int figure_param_dialog(Fl_Widget * f,bool adjust,double & tmin,double & tmax,double & tcurrent,double & tstep,string & name,bool symb,string & tmp,bool ispnt){
    if (!Xcas_MainTab || !Xcas_MainTab->visible()){ fl_alert("Please finish current task"); return 0; }
#ifdef EMCC2
    Xcas_main_menu_hide();
#endif
#ifdef MODAL_GROUP
    ...
    static Fl_Button * button1 =0; // cancel
    static string paramname,pointname;
    if (f){
      int l=f->labelsize();
      if (!w){
	...
	wname->tooltip(gettext(ispnt?"Point variable name":"Parameter variable name"));
	paramname="a"; pointname="A";
	...
	change_group_fontsize(w,l);
	w->resizable(w);
	w->label(gettext(ispnt?"Symbolic point definition":"Parameter definition"));
      }
#ifdef MODAL_GROUP
	...
	wstep->value(tstep);
	pointname=paramname=name;
	wsymb->value(symb);
      }
      if (ispnt){
        wname->value(pointname.c_str());
	wcurrent->value(tcurrent);        
      }
      else
	...
	tstep=wstep->value();
        if (ispnt)
	  tmp="assume("+string(wname->value())+"x=["+print_DOUBLE_(tcurrent)+","+print_DOUBLE_(tmin)+","+print_DOUBLE_(tmax)+","+print_DOUBLE_(tstep)+"])";
	else if (wsymb->value())
  ...
  static void cb_Figure_Parameter(Fl_Widget * m , void*) {
	...
	return;
      ...
      if (figure_param_dialog(f,false,tmin,tmax,tcur,tstep,name,false,tmp,false)){
	History_Pack * hp=f->geo->hp;
	int pos;
	if (hp!=get_history_pack(wid,pos))
	  pos=hp->children()-1;
  ...
  static void cb_Figure_Symbpoint(Fl_Widget * m , void*) {
    Fl_Widget * wid=Fl::focus();
    Figure * f=find_figure(m);
    if (f){
      if (!f->geo->hp)
	return;
      string tmp,name;
      double tmin,tmax,tcur,tstep;
      set_pminmax(f->geo->window_xmin,f->geo->window_xmax,tmin,tmax);
      int rr=std_rand();
      rr=(rr*50.0)/RAND_MAX-25;
      tcur=(tmin+tmax)/2+(tmax-tmin)/50*rr;
      if (figure_param_dialog(f,false,tmin,tmax,tcur,tstep,name,false,tmp,true/*ispnt*/)){
	History_Pack * hp=f->geo->hp;
	int pos;
	if (hp!=get_history_pack(wid,pos))
	  pos=hp->children()-1;
	...
	hp->set_value(pos,tmp,true);
        ...
        rr=(rr*50.0)/RAND_MAX-25;
        tcur=(tmin+tmax)/2+(tmax-tmin)/50*rr;
        tmp="assume("+name+"y=["+print_DOUBLE_(tcur)+","+print_DOUBLE_(tmin)+","+print_DOUBLE_(tmax)+","+print_DOUBLE_(tstep)+"])";
	hp->add_entry(pos);
	hp->set_value(pos,tmp,true);
        ++pos;
        bool is3d=dynamic_cast<Graph3d *>(f->geo);
        if (is3d){
          int rr=std_rand();
          rr=(rr*50.0)/RAND_MAX-25;
          tcur=(tmin+tmax)/2+(tmax-tmin)/50*rr;
          tmp="assume("+name+"z=["+print_DOUBLE_(tcur)+","+print_DOUBLE_(tmin)+","+print_DOUBLE_(tmax)+","+print_DOUBLE_(tstep)+"])";
          hp->add_entry(pos);
          hp->set_value(pos,tmp,true);
          ++pos;
        }
        tmp=name+":=point("+name+"x,"+name+"y";
        if (is3d)
          tmp+=","+name+"z";
        tmp+=")";
        hp->add_entry(pos);
  ...
  int plotparam_dialog(Fl_Widget * spread_ptr,std::string & arg,int modeplot){
    ...
    Fl_Window * mainw=xcas::Xcas_Main_Window;
    if (mainw){
      int X=Xcas_MainTab->x(),Y=Xcas_MainTab->y(),W=Xcas_MainTab->w(),H=Xcas_MainTab->h(),L=Xcas_MainTab->labelsize();
      if (H>12*L) H=12*L;
      if (Xcas_MainTab){
        w->resize(X,Y,W,H);
        Xcas_MainTab->hide();
      ...
      w->hotspot(w);
#endif        
    // w->set_modal(); w->show();    w->hotspot(w);
    autosave_disabled=true;
    Fl::focus(button0);
    for (;;) {
      Fl_Widget *o = Fl::readqueue();
  ...
  */
  ...
  Fl_Menu_Item Figure_menu[] = {
    ...
    {gettext("Add symbolic point"), 0x80069,  (Fl_Callback *) cb_Figure_Symbpoint, 0, 0, 0, 0, 14, 56},
  ...
  Figure::Figure(int X,int Y,int W,int H,int L,bool dim3):Fl_Tile(X,Y,W,H),namestr(0){
    ...
    Fl_Menu_Bar * menubar = new Fl_Menu_Bar(X,Y,Max(W/3,80),l);
    ...
    int x=X;
    x += menubar->w();
    mode = new Fl_Output(x,Y,3*l,l);
    x += mode->w();
    mode->value(gettext("Frame"));
    Fl_Menu_Button * menubut = new Fl_Menu_Button(x,Y,3*l,l);
    ...
    x += checkb->w();
#endif
    checkdisp = new Fl_Check_Button(x,Y,3*l,l,"Landscape");
    checkdisp->align(FL_ALIGN_CLIP | FL_ALIGN_LEFT | FL_ALIGN_INSIDE);
  ...
  Logo::Logo(int X,int Y,int W,int H,int L):Fl_Tile(X,Y,W,H){
    ...
    if (L<30)
      L=30;
    bool noed=W<=noed_maxsize;
    Fl_Group::current(this);
    xcas::HScroll * s = new xcas::HScroll (X,Y,W/(noed?2:4),H);
    s->box(FL_FLAT_BOX);
    hp = new History_Pack (X,Y,max(W/(noed?2:4)-20,W/5),max(H-L-20,20));
    // hp->new_question=new_question_multiline_input;
    hp->labelsize(L);
    hp->pretty_output=false;
    hp->eval_below=false;
    ...
    hp->end();
    s->end();
    t =  new Turtle(s->x()+s->w(),Y,W/2,H-L);
    t->labelsize(L);
    ...
    button_group = new Fl_Group(t->x(),Y+H-L,bw*nbut,L);
    ...
    ed = new Editeur(t->x()+t->w(),Y,noed?0:W/4,H);
    ...
    {
      disposition=1;
      s->resize(X,Y,W/2,2*H/3);
      hp->Fl_Group::resize(X,Y,max(W/2-20,W/3),2*H/3);
      hp->resize();
      t->resize(X+W/2,Y,W/2,2*H/3-L);
      int bw=(W/2-5*L/2)/14*14; 
      button_group->resize(X+W/2,t->y()+t->h(),bw,L);
      menubar->resize(X+W/2+bw,button_group->y(),W/2-bw,L);
      ed->resize(X,menubar->y()+menubar->h(),W,H-s->h());
    }
#else
    disposition=0;
#endif
  ...
  void Logo::resize(int X,int Y,int W,int H){
    ...
    if (disposition==1){
      g->resize(X,Y,W/2,2*H/3);
      //hp->Fl_Group::resize(X,Y,max(W/2-20,W/3),2*H/3);
      //hp->resize();
      t->resize(X+W/2,Y,W/2,2*H/3-L);
      int bw=(W/2-2*L)/14*14; 
      button_group->resize(X+W/2,t->y()+t->h(),bw,L);
      menubar->resize(X+W/2+bw,button_group->y(),W/2-bw,L);
      ed->resize(X,menubar->y()+menubar->h(),W,H-g->h());
      init_sizes();
    }
    else {
      int hph=hp->h();
      g->resize(X,Y,W/(noed?2:4),H);
      hp->Fl_Group::resize(X,Y,max(W/(noed?2:4)-20,W/5),hph);
      hp->resize();
      t->resize(g->x()+g->w(),Y,W/2,H-L);
      int bw=(W/2-5*L/2)/14*14; 
      button_group->resize(t->x(),Y+H-L,bw,L);
      menubar->resize(button_group->x()+button_group->w(),Y+H-L,W/2-bw,L);
      ed->resize(X+W/4+W/2,Y,noed?0:W/4,H);
    }
  }
  ...
  int Gen_Value_Slider::handle(int event){
	...
	if (Multiline_Input_tab * m=dynamic_cast<Multiline_Input_tab *>(wid)){
	  gen g=m->g();
	  if (figure_param_dialog(hp,true,tmin,tmax,tcur,tstep,paramname,g.is_symb_of_sommet(at_assume),tmp,false) ){
	    minimum(tmin);
	    maximum(tmax);
	    value(tcur);
