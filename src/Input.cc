...
namespace xcas {
  ...
  void * Xcas_main_menu_ptr=0;
  
  void Xcas_main_menu_show(){
    if (Fl_Menu_Bar * ptr=(Fl_Menu_Bar * )Xcas_main_menu_ptr)
      ptr->show();
  }
  
  void Xcas_main_menu_hide(){
    if (Fl_Menu_Bar * ptr=(Fl_Menu_Bar * )Xcas_main_menu_ptr)
      ptr->hide();
  }
...
#define TAB_ARGS 6
...
#if defined __APPLE__ || defined WIN32 || defined EMCC2
  ...
  string doc_prefix("doc/fr/");
  void local_help(const string & s){
#ifdef EMCC2
      if (xcas::Xcas_help_window){
	if (Xcas_Main_Window){
	  int X=Xcas_Main_Window->x(),Y=Xcas_Main_Window->y(),W=Xcas_Main_Window->w(),H=Xcas_Main_Window->h();
	  Xcas_help_window->resize(X+W/10,Y+H/10,4*W/5,4*H/5);
	}
	else
	  xcas::Xcas_help_window->position(50,50);
	xcas::Xcas_help_window->load(("/"+doc_prefix+s).c_str());
	xcas::Xcas_help_window->show();
	return;
      }
#endif
    if (xcas::use_external_browser){
      giac::system_browser_command(doc_prefix+s);
    }
    else {
      if (xcas::Xcas_help_window){
	xcas::Xcas_help_window->load((giac::giac_aide_dir()+doc_prefix+s).c_str());
	xcas::Xcas_help_window->show();
      }
    }
  }
  ...
  void update_examples(const string & s,Fl_Browser * examples,Fl_Browser * related,Fl_Browser * syns,Fl_Output * output,Fl_Input ** argtab,int language){
	...
	int L=output->labelsize()+4;
	int eh=L*((1+(ptr-argtab))/2)+8;
  ...
  void browser_html_help(Fl_Browser * b,Fl_Browser * examples,Fl_Browser * related,Fl_Browser * syns,Fl_Output * output,Fl_Input ** argtab,int language){
    int k=b->value();
    //printf("browser_html_help external=%i k=%i\n",use_external_browser,k);
    if (k>=1){
      if (Xcas_help_window){
	string s=b->text(k);
  ...
  int handle_tab(const string & s,const vector<string> & v_orig,int dx,int dy,int & remove,string & ans,bool allow_immediate_out){
    ...
    if (!handle_tab_w){
      ...
      handle_tab_w=new Fl_Group(0,0,dx,dy);
      modal_w_tab.push_back(handle_tab_w);
      ...
      invisible = new Fl_Box(button0->x(),button0->y()+button0->h(),dx,
#ifdef EMCC2
                             2
#else
                             L+2
      ...
      output = new Fl_Multiline_Output(2,input->y()+input->h(),dx-4,
#ifdef EMCC2
                                       2*L+9
#else
                                       3*L+9
      ...
      output->labeltype(FL_NO_LABEL);
      // arguments
      int ypos=output->y()+output->h();
      int argL=L+4;
      for (int j=0;j<TAB_ARGS;j++){
	argtab[j]= new Fl_Input(dx/4+(j%2)*dx/2,ypos+1+(j/2)*argL,dx/4,argL-1);
	argtab[j]->when(FL_WHEN_ENTER_KEY |FL_WHEN_NOT_CHANGED);
      }
      ypos += 3*argL;
      examples = new Fl_Hold_Browser(output->x(),ypos,output->w(),handle_tab_w->h()-output->y()-output->h()-2-3*argL);
      examples->label("Examples");
      examples->type(2);
      examples->align(FL_ALIGN_LEFT);
      examples->labeltype(FL_NO_LABEL);
	...
	if (Xcas_MainTab)
      ...
      Fl::focus(input);
      //int ck=browser->value();
      browser->value(i);
      for (;;) {
	if (!handle_tab_w->visible())
          handle_tab_w->show();
#if 0 // ndef EMCC2
